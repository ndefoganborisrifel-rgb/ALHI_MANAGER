          ...
          <div key={sem} style={{ background: "var(--bg-card)", borderRadius: "12px", border: "1px solid var(--border)", overflow: "hidden" }}>
            <div style={{ padding: "10px 16px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: "10px", background: "var(--bg-muted)" }}>
              ...
              <span style={{ padding: "2px 8px", background: "#2563eb20", color: "#2563eb", borderRadius: "20px", fontSize: "11px", fontWeight: 600 }}>
                {list.length} matiere{list.length > 1 ? "s" : ""}
              </span>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
                <thead>
                  <tr style={{ background: "var(--bg-muted)" }}>
                    {["Code", "Intitule", "UE", "Credits", "Heures", "Enseignant"].map((h) => (
                    ...
                    <tr key={course.id} style={{ borderTop: "1px solid var(--border-muted)" }}>
                      ...
                      <td style={{ padding: "10px 14px" }}>
                        {course.teacher ? (
                          ...
                          <span style={{ fontSize: "11px", color: "var(--text-muted)", fontStyle: "italic" }}>Non assigne</span>
                        )}
                      </td>
                    </tr>
                  ))}
      ...
      <div style={{ maxWidth: "1200px" }}>
        <div style={{ marginBottom: "20px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "3px" }}>
            ...
            <h1 style={{ fontSize: "22px", fontWeight: 800, color: "var(--text)" }}>Mes Matieres</h1>
          </div>
          <p style={{ fontSize: "13px", color: "var(--text-secondary)" }}>
            Liste des cours de votre filiere pour l&apos;annee 2025-2026
          </p>
        </div>
        ...
        {!student?.filiere ? (
          <div style={{ background: "var(--bg-card)", borderRadius: "12px", border: "1px solid var(--border)", padding: "60px", textAlign: "center" }}>
            ...
            <p style={{ fontSize: "15px", fontWeight: 600, color: "var(--text)", marginBottom: "6px" }}>Aucune filiere associee</p>
            <p style={{ fontSize: "13px", color: "var(--text-muted)" }}>Votre compte n&apos;est pas encore lie a une filiere.</p>
          </div>
        ) : !student.filiere.coursesPublished ? (
          <div style={{ background: "var(--bg-card)", borderRadius: "12px", border: "1px solid var(--border)", padding: "60px", textAlign: "center" }}>
            <Lock style={{ width: "40px", height: "40px", color: "var(--text-muted)", margin: "0 auto 12px" }} />
            <p style={{ fontSize: "15px", fontWeight: 600, color: "var(--text)", marginBottom: "6px" }}>Liste non encore publiee</p>
            <p style={{ fontSize: "13px", color: "var(--text-muted)" }}>
              La liste des matieres de la filiere <strong>{student.filiere.name}</strong> n&apos;est pas encore disponible.
            </p>
          </div>
        ) : (
          <>
  ...
  return (
    <div style={{ maxWidth: "1200px" }}>
      <div style={{ marginBottom: "20px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "3px" }}>
          <BookMarked style={{ width: "22px", height: "22px", color: "#B91C2F" }} />
          <h1 style={{ fontSize: "22px", fontWeight: 800, color: "var(--text)" }}>Matieres de mes enfants</h1>
        </div>
        <p style={{ fontSize: "13px", color: "var(--text-secondary)" }}>
          Cours et enseignants pour chaque enfant inscrit, annee 2025-2026
        </p>
      </div>
      ...
      {!parent?.students.length ? (
        <div style={{ background: "var(--bg-card)", borderRadius: "12px", border: "1px solid var(--border)", padding: "60px", textAlign: "center" }}>
          <BookMarked style={{ width: "40px", height: "40px", color: "var(--text-muted)", margin: "0 auto 12px" }} />
          <p style={{ fontSize: "15px", fontWeight: 600, color: "var(--text)", marginBottom: "6px" }}>Aucun enfant associe</p>
          <p style={{ fontSize: "13px", color: "var(--text-muted)" }}>Votre compte parent n&apos;est pas encore lie a un etudiant.</p>
        </div>
      ) : (
            ...
            <div key={child.id}>
              ...
              {!child.filiere ? (
                ...
                <div style={{ background: "var(--bg-card)", borderRadius: "10px", border: "1px dashed var(--border)", padding: "24px", textAlign: "center", color: "var(--text-muted)", fontSize: "13px" }}>
                  ...
                  Liste des matieres non encore publiee.
                </div>
              ) : (
  ...
  if (courses.length === 0) {
    return (
      <div style={{ background: "var(--bg-card)", borderRadius: "12px", border: "1px solid var(--border)", padding: "40px", textAlign: "center", color: "var(--text-muted)", fontSize: "13px" }}>
        Aucune matiere enregistree pour cette filiere.
      </div>
    );
  }
